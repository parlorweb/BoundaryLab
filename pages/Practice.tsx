  ...
  if (!session || questions.length === 0) {
    return (
      <div className="max-w-md mx-auto py-20 text-center animate-in zoom-in duration-300 px-6">
        ...
        <button 
          onClick={onExit}
          className="mt-8 w-full px-6 py-3.5 bg-slate-900 text-white rounded-xl font-black hover:bg-slate-800 transition-all active:scale-95 flex items-center justify-center gap-2 text-sm"
        >
          <ArrowLeft size={16} />
          Back to Dashboard
        </button>
  ...
  if (isEnded) {
    return (
      <div className="max-w-xl mx-auto py-12 text-center animate-in zoom-in slide-in-from-bottom-8 duration-500 px-6">
        <div className="w-20 h-20 bg-gradient-to-tr from-blue-600 to-indigo-600 rounded-[28px] flex items-center justify-center text-white mx-auto mb-8 shadow-xl shadow-blue-100">
        ...
        <div className="mt-10 space-y-3">
          <button onClick={onExit} className="w-full px-10 py-4 bg-slate-900 text-white rounded-xl font-black hover:bg-slate-800 transition-all shadow-lg text-sm">Dashboard Overview</button>
          <button onClick={handleRestart} className="w-full px-10 py-4 bg-white border border-slate-200 text-slate-900 rounded-xl font-black hover:bg-slate-50 transition-all text-sm">Start New Quiz</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 pb-16 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
            ...
            {currentQuestion.type === 'fill' ? (
              ...
              <div className="space-y-2">
                {currentQuestion.choices.map((choice) => {
                  ...
                  return (
                    <div key={choice.id} className={`flex flex-col sm:flex-row gap-3 items-center p-3 bg-white border border-slate-100 rounded-[16px] shadow-sm transition-all ${hasAnswered ? (isMatchCorrect ? 'border-emerald-200 bg-emerald-50/20' : 'border-red-200 bg-red-50/20') : 'hover:border-slate-200'}`}>
                      <div className="flex-1 font-bold text-slate-800 text-sm px-1">{choice.text}</div>
                      <div className="shrink-0 w-full sm:w-48">
                        <select 
                          disabled={hasAnswered}
                          className="w-full p-2.5 border border-slate-200 rounded-lg font-bold text-xs"
                          value={currentValue}
              ...
              <div className="grid grid-cols-1 gap-2">
                {currentQuestion.choices.map((choice) => {
                  ...
                  let cardStyle = "bg-white border-slate-100 text-slate-700 hover:border-slate-300";
                  if (isSelected && !hasAnswered) cardStyle = "bg-blue-600 border-blue-600 text-white shadow-lg shadow-blue-50";
                  if (hasAnswered) {
                    ...
                    else cardStyle = "bg-white border-slate-100 text-slate-400 opacity-50";
                  }
                    ...
                    >
                      <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-[10px] font-black mr-3 shrink-0 transition-all ${
                        isSelected || (hasAnswered && isCorrect) 
                          ...
                          : 'bg-slate-50 text-slate-400 group-hover:bg-blue-50'
                      }`}>
                        {choice.choiceKey}
                      </div>
                      <span className="flex-1 font-bold text-sm leading-snug">{choice.text}</span>
                      <div className="shrink-0 ml-3">
                        {hasAnswered && isCorrect && <CheckCircle2 className="text-white" size={20} strokeWidth={3} />}
                        {hasAnswered && !isCorrect && isSelected && <XCircle className="text-white" size={20} strokeWidth={3} />}
                      </div>
                    </button>
                  );
                })}
          ...
          <div className="pt-2 flex flex-col sm:flex-row items-center gap-3">
            {!currentAnswer ? (
              <button
                ...
                className="w-full sm:w-auto px-8 py-3.5 bg-blue-600 text-white rounded-xl font-black text-sm hover:bg-blue-700 disabled:opacity-30 transition-all shadow-lg active:scale-95"
              >
                Confirm Answer
              </button>
            ) : (
              <button
                onClick={handleNext}
                className="w-full sm:w-auto px-8 py-3.5 bg-slate-900 text-white rounded-xl font-black text-sm hover:bg-slate-800 transition-all shadow-xl active:scale-95 flex items-center justify-center gap-2 group"
              >
            ...
            {!currentAnswer && (
              <button 
                ...
                className="text-[10px] font-black text-slate-400 hover:text-blue-600 flex items-center gap-2 px-4 py-3 rounded-lg hover:bg-white transition-all uppercase tracking-widest"
              >
          ...
          {currentAnswer && (
            <div className={`p-5 md:p-6 rounded-[24px] border-2 animate-in slide-in-from-top-4 duration-300 shadow-sm ${currentAnswer.isCorrect ? 'bg-emerald-50 border-emerald-100' : 'bg-red-50 border-red-100'}`}>
              <div className="flex items-center gap-2 mb-3">
                <div className={`p-2 rounded-lg ${currentAnswer.isCorrect ? 'bg-emerald-500 text-white' : 'bg-red-500 text-white'}`}>
        ...
        <div className="lg:col-span-4 space-y-4 lg:sticky lg:top-20 h-fit">
          <Card title="Concept Mastery" className="bg-white/50 border-white">
            <div className="text-center pb-4 border-b border-slate-100 mb-4">
              <h5 className="text-sm font-black text-slate-900 leading-tight mb-2 px-1 truncate">{currentConcept?.name || 'Knowledge Unit'}</h5>
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-slate-600 leading-relaxed font-semibold bg-slate-50/50 p-2.5 rounded-lg border border-slate-100">{currentConcept?.summary}</p>
              </div>
              
              {currentConcept?.formula && (
                <div className="bg-blue-600 p-3.5 rounded-[16px] shadow-md shadow-blue-50">
                  <div className="flex items-center gap-1.5 mb-1.5">
                    <Calculator size={10} className="text-blue-200" />
                    <span className="text-[8px] font-black text-blue-100 uppercase tracking-widest">Formula</span>
                  </div>
                  <p className="text-xs font-mono text-white font-black break-all text-center leading-tight">{currentConcept.formula}</p>
                </div>
              )}
            </div>
          </Card>
          ...
          {showHint && (
            <div className="p-4 bg-amber-50 border-2 border-amber-100 rounded-[20px] animate-in zoom-in-95 duration-200 shadow-lg shadow-amber-900/5">
              ...
              <button onClick={() => setShowHint(false)} className="mt-3 text-[8px] font-black text-amber-700 uppercase tracking-widest hover:underline">Close</button>
            </div>
          )}
        </div>
      </div>
